...
RSpec.describe MermaidRailsErd::RelationshipBuilders::BelongsToRelationshipBuilder do
  ...
  describe "#build" do
    context "with standard belongs_to association" do
      ...
      before do
        allow(assoc).to receive_messages(name: "post", foreign_key: "post_id", options: {}, macro: :belongs_to)
    ...
    context "when foreign key cannot be determined" do
      ...
      before do
        allow(assoc).to receive_messages(name: "post", options: {}, macro: :belongs_to)
    ...
    context "with duplicate one-to-one relationship" do
      ...
      before do
        ...
        allow(assoc).to receive_messages(name: "profile", foreign_key: "profile_id", options: {}, macro: :belongs_to)

        to_table_info = { table_name: "profiles", primary_key: "id" }
        allow(association_resolver).to receive(:resolve).with(assoc).and_return(to_table_info)

        # Set up second relationship (Profile belongs_to User)
        allow(other_assoc).to receive_messages(name: "user", foreign_key: "user_id", options: {}, macro: :belongs_to)
