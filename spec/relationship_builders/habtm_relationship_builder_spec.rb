  ...
  describe "#build" do
    context "with standard HABTM association" do
      ...
      before do
        ...
        allow(model).to receive_messages(table_name: "users", name: "User", primary_key: "id")

        # Set up the association properties
        allow(assoc).to receive_messages(name: "roles", options: {}, macro: :has_and_belongs_to_many, join_table: "roles_users", foreign_key: "user_id", association_foreign_key: "role_id")
    ...
    context "with custom join table name" do
      ...
      before do
        ...
        allow(assoc).to receive_messages(name: "projects", options: { join_table: "project_assignments" }, macro: :has_and_belongs_to_many, join_table: "project_assignments", foreign_key: "user_id", association_foreign_key: "project_id")
    ...
    context "when join table does not exist" do
      ...
      before do
        # Set up the model being tested
        allow(model).to receive_messages(name: "User", table_name: "users")

        # Set up the association properties
        allow(assoc).to receive_messages(name: "roles", options: {}, macro: :has_and_belongs_to_many, join_table: "roles_users")
